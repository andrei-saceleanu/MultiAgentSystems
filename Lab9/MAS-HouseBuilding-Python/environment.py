            ...
            if self._crt_item_idx < len(self._construction_items):
                ...
                if self._negotiation_status[negotiation_item]["completed"]:
                    ...
                    if best_response_ag:
                        ...
                        self.acme_profit += self._owner_agent.budget_dict[negotiation_item] - best_response
...
if __name__ == "__main__":
    ...
    print(f"Budgets: {env._owner_agent.budget_dict}")
    print(f"ACME profit: {env.acme_profit}")
    for ag in env._company_agents:
        print(f"{ag.name} -> {ag.contract_wins} contracts")
        for k, v in ag.profit.items():
            print(f"{k} - Cost: {v[1]} Received: {v[0]} Profit: {v[0]-v[1]}")
        print()
